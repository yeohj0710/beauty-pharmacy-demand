...
`brief`가 **조사 규칙 · 오늘의 작업 목록 · 완료 파이프라인**을 한 번에
출력한다. 제품 명단은 현재 데이터에서 계산하므로, 문서에 적힌 옛 명단을 찾지
말고 항상 이 출력값을 작업 목록으로 삼는다. (`-- --json`을 붙이면 기계가 읽는
형식으로 나온다.)
...
- **전체 갱신** — 마지막 수집으로부터 30일이 지나면 brief가 알려준다. 이때는
  5개 채널을 전부 다시 수집한다.
